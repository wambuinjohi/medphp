...
The application now includes a comprehensive **Network Diagnostics Tool** to help identify and resolve connectivity issues with the `accounts.heal.co.ke` API server.
...
### 1. **DNS Resolution**
- Tests if the domain `accounts.heal.co.ke` can be resolved to an IP address
...
**Error Message**: "Failed to resolve accounts.heal.co.ke"
...
**Solutions**:
1. Check if `accounts.heal.co.ke` is online
...
2. Check firewall rules allow HTTPS (port 443) to accounts.heal.co.ke
3. Verify domain is correct: `accounts.heal.co.ke`
4. Test with: `curl -I https://accounts.heal.co.ke`
...
**Solutions**:
1. Verify proxy allows HTTPS to `accounts.heal.co.ke`
...
curl -I https://accounts.heal.co.ke

# Test health endpoint
curl https://accounts.heal.co.ke/api.php?action=health

# Test with verbose output
curl -v https://accounts.heal.co.ke/api.php?action=health
```
...
curl -v https://accounts.heal.co.ke
...
tcpdump -i any -n "host accounts.heal.co.ke"
...
**API Domain**: accounts.heal.co.ke  
