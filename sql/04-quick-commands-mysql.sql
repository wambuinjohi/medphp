-- =====================================================
-- QUICK COMMANDS FOR COMMON OPERATIONS (MYSQL VERSION)
-- =====================================================
...
SELECT 
  ...
  c.created_at,
  COUNT(DISTINCT i.id) as total_invoices,
  COALESCE(SUM(i.total_amount), 0) as total_purchase_value
FROM customers c
...
SELECT 
  ...
  CASE 
    ...
    WHEN i.due_date = CURDATE() AND i.balance_due > 0 THEN 'DUE TODAY'
    WHEN i.due_date > CURDATE() AND i.balance_due > 0 THEN 'PENDING'
    ELSE 'OK'
...
-- =====================================================
-- 17. SOFT DELETE COMPANY (keeps data, marks inactive)
-- =====================================================
...
-- =====================================================
-- 19. EXPORT COMPANY DATA (for backup/audit)
-- =====================================================
-- MySQL doesn't have direct COPY TO feature, use SELECT INTO OUTFILE instead:
-- SELECT 
--   c.id,
--   c.name,
--   c.email,
...
-- WHERE c.is_active = true
-- ORDER BY c.created_at DESC
-- INTO OUTFILE '/tmp/companies_export.csv'
-- FIELDS TERMINATED BY ',' ENCLOSED BY '"' LINE TERMINATED BY '\n';
