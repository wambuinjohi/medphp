...
**Expected Output:**
```
🌐 Using REMOTE API at https://accounts.heal.co.ke
📡 [POST] health - Starting request...
✅ External API health check passed: https://accounts.heal.co.ke/api.php
```

**If CORS Error:**
```
❌ Network Error for health on API
API Endpoint: https://accounts.heal.co.ke/api.php
🔍 Troubleshooting:
...
Ensure the backend at `https://accounts.heal.co.ke/api.php` is configured with:
...
3. Test with curl command:
   ```bash
   curl -i -X OPTIONS https://accounts.heal.co.ke/api.php?action=health
   ```
