...
export async function initializeExternalAPI(options: SetupOptions = {}): Promise<SetupResult> {
  const {
    apiUrl = import.meta.env.VITE_EXTERNAL_API_URL || 'https://med.layonsconstruction.com/api.php',
    email,
    password,
    onProgress,
  } = options;
...
export async function checkAdminExists(options: SetupOptions = {}): Promise<boolean> {
  const apiUrl =
    options.apiUrl || import.meta.env.VITE_EXTERNAL_API_URL || 'https://med.layonsconstruction.com/api.php';
