...
export async function testDNSResolution(): Promise<DiagnosticResult> {
  try {
    console.log('🔍 Testing DNS resolution for accounts.heal.co.ke');

    // Use public DNS API
    const response = await fetch('https://dns.google/resolve?name=accounts.heal.co.ke', {
    ...
    if (response.ok) {
      ...
      return {
        ...
        details: { domain: 'accounts.heal.co.ke', hasAnswers },
      };
    } else {
      return {
        test: 'DNS Resolution',
...
export function getTroubleshootingSuggestions(errorType: string): string[] {
  const suggestions: Record<string, string[]> = {
    failed_to_fetch: [
      ...
      '6. Verify backend server is running at https://accounts.heal.co.ke',
    ],
    timeout: [
