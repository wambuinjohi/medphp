...
export async function testDirectConnectivity(
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      ...
      recommendations.push('💡 Check if med.layonsconstruction.com is online');
      ...
      recommendations.push('💡 Check if corporate firewall blocks med.layonsconstruction.com');
    } else if (error.message.includes('ENOTFOUND') || error.message.includes('getaddrinfo')) {
      message = 'DNS resolution failed for med.layonsconstruction.com';
      recommendations.push('❌ Cannot resolve domain name');
      recommendations.push('💡 Check DNS server configuration');
      recommendations.push('💡 Verify domain med.layonsconstruction.com is valid');
    ...
    if (viaHeader || xForwardedFor) {
      return {
        ...
        recommendations: [
          ...
          '💡 Verify proxy allows HTTPS connections to med.layonsconstruction.com',
        ],
      };
    }
...
export async function testDNSResolution(
  domain: string = 'med.layonsconstruction.com'
): Promise<NetworkDiagnostic> {
  try {
...
export function generateNetworkDiagnosticReport(results: NetworkDiagnostic[]): string {
  ...
  report += '║         med.layonsconstruction.com Connectivity              ║\n';
  report += '╚════════════════════════════════════════════════════════════╝\n\n';
  ...
  if (errors.length > 0) {
    ...
    if (dnsError) {
      ...
      report += `   4. Verify domain med.layonsconstruction.com is correct\n\n`;
    }

    if (connectError) {
      report += `🔴 Connection Issue Detected\n`;
      report += `   1. Check if firewall allows HTTPS (port 443) to med.layonsconstruction.com\n`;
      ...
      report += `   5. Check server status: is med.layonsconstruction.com online?\n\n`;
    }
