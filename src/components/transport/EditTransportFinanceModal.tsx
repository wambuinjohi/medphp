...
import { useUpdateTransportFinance } from '@/hooks/useTransport';
...
interface EditTransportFinanceModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  finance: TransportFinance;
  onSuccess: () => void;
  companyId: string;
  drivers: Driver[];
  vehicles: Vehicle[];
...
export function EditTransportFinanceModal({
  open,
  onOpenChange,
  finance,
  onSuccess,
  ...
  vehicles,
  materials,
}: EditTransportFinanceModalProps) {
  ...
  const updateFinance = useUpdateTransportFinance();

  useEffect(() => {
    if (finance && open) {
      setFormData({
        vehicle_id: finance.vehicle_id || '',
        material_id: finance.material_id || '',
        buying_price: finance.buying_price?.toString() || '',
        fuel_cost: finance.fuel_cost?.toString() || '',
        driver_fees: finance.driver_fees?.toString() || '',
        other_expenses: finance.other_expenses?.toString() || '',
        selling_price: finance.selling_price?.toString() || '',
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Edit Transport Finance Record</DialogTitle>
          <DialogDescription>
            Update the transport and financial details below
          </DialogDescription>
        </DialogHeader>
